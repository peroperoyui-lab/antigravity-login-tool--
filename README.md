...
预计这个项目不需要长期维护，antigravity一直都是oauth没走代理的问题，只需要在无效时再排查一遍即可。
...
This project is expected to require long-term maintenance; the `antigravity` has consistently stemmed from OAuth bypassing the proxy, so it only needs to be investigated again if it fails.
